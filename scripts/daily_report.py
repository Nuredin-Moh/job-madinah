...
def render_text(s):
    ...
    days_left = s["queue_depth"] / 17
    L.append(f"  Reserve de cibles restante: {s['queue_depth']} (= {days_left:.1f} jours a 17/jour)")
    if s["queue_depth"] < 17:
        L.append("  !!! RESERVE VIDE : DEMAIN 07:00 IL N'Y AURA PAS D'ENVOI. Recharger data/companies.csv ce soir.")
    elif days_left < 3:
...
def main():
    ...
    # L'alerte doit se voir dans la boite de reception sans ouvrir le mail :
    # le 12.08.2026 la file etait vide et le corps du rapport le disait deja,
    # mais noye dans un sujet identique a tous les autres jours.
    if summary["queue_depth"] < 17:
        subject = "!!! RESERVE VIDE — pas d'envoi demain !!! " + subject
    elif summary["queue_depth"] / 17 < 3:
        subject = "/!\\ Reserve basse — " + subject
